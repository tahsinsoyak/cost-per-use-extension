...
- Run `npm run release:check` for the complete pipeline, including real extension smoke tests. The individual stages are listed below.
- Run `npm test`.
- Run `npm run build`.
...
- Confirm the dashboard recognizes package version `1.1.0`.
...
- Add the no-login reviewer instructions from `releases/v1.1.0.md`; leave username and password blank.

## Final review

- Verify the Patreon link is voluntary and does not unlock features.
- Confirm every listing claim matches version `1.1.0`.
