...
export default function Website() {
  ...
  const messages=lang==='en'?{
    positive:'Enter a number greater than zero.', nonnegative:'Enter zero or a positive number.',
    whole:'Enter a whole number of installments.', paired:'Enter both the installment count and total paid.',
    ...
    positive:'Sıfırdan büyük bir sayı girin.', nonnegative:'Sıfır veya pozitif bir sayı girin.',
    whole:'Taksit sayısını tam sayı olarak girin.', paired:'Hem taksit sayısını hem de toplam ödemeyi girin.',
    resaleLimit:'Satış değeri, toplam ödeme ve bakım maliyetini aşamaz.',
    usageRange:'En az bir kullanım ve makul bir toplam için süreyi veya sıklığı ayarlayın.'
  };
  ...
  function chooseProduct(example:ProductExample) {
    ...
    setCurrency('USD');setTarget('');setName(example[lang]);setPreset(-1);setAdvanced(false);
    ...
    <main>
      ...
      <section id="calculator" tabIndex={-1} aria-label={t.nav[0]} className="calculator container">
        ...
        <div className="calculator-inputs">
          <div className="section-heading"><div><h2>{t.calc}</h2></div><span className="live-dot">{lang==='en'?'LIVE ESTIMATE':'ANLIK TAHMİN'}</span></div>
          ...
          <div className="purchase-fields"><label className="field product-field"><span>{t.product}</span><input value={name??t.examples[preset]??''} maxLength={100} onChange={e=>setName(e.target.value)} /></label>
          ...
          </div><div className="habit-fields"><div className="duration-control"><div className="duration-row"><span>{t.duration}</span><div className="stepper"><button aria-label={lang==='en'?'Decrease years':'Yıl azalt'} disabled={years<=1} onClick={()=>update('years',String(Math.max(1,years-1)))}><Minus size={16}/></button><label><input {...errorProps('years')} aria-label={t.duration} type="number" min="0.1" step="any" value={inputs.years} onChange={e=>update('years',e.target.value)}/><span>{t.years}</span></label><button aria-label={lang==='en'?'Increase years':'Yıl artır'} onClick={()=>update('years',String(years+1))}><Plus size={16}/></button></div></div>
          ...
          <button className="advanced-toggle" aria-expanded={advanced} aria-controls="advanced-fields" onClick={()=>setAdvanced(!advanced)}>{advanced?<Minus size={16}/>:<Plus size={16}/>} {t.advanced}{advancedErrors&&<span className="validation-badge">{lang==='en'?'Check fields':'Alanları kontrol edin'}</span>}</button>
          ...
          {!usable&&<p className="error" role="alert">{lang==='en'?'Check the highlighted fields to see your estimate.':'Tahmini görmek için işaretli alanları kontrol edin.'}</p>}
        </div>
        <div id="estimate-details" className="calculator-result">
          ...
          <div className="result-main" aria-live="polite"><p>{name??t.examples[preset]??''}</p><div className="result-amount"><span className="sr-only">{displayedCost}</span><SmoothAmount value={usable?.costPerUse??null} currency={currency} lang={lang}/></div><span>{t.perUse}</span></div>
          ...
          <details className="estimate-extras"><summary>{lang==='en'?'Compare usage & set a target':'Kullanımı karşılaştır ve hedef belirle'}<Plus size={16}/></summary><div className="scenario"><h3>{t.less}</h3><div className="scenario-row"><span>{t.expected}</span><div className="bar expected"/><strong>{usable?money(usable.costPerUse):'—'}</strong></div><div className="scenario-row"><span>{t.half}</span><div className="bar half"/><strong>{scenarios?money(scenarios.halfUsage):'—'}</strong></div><p>{t.scenario}</p></div>
      ...
      <section id="extension" className="extension-section"><div className="container extension-grid"><div><p className="eyebrow">{t.extensionTag}</p><h2>{t.extensionTitle}</h2><p>{t.extensionText}</p><ul>{t.bullets.map(b=><li key={b}><Check size={18}/>{b}</li>)}</ul><a className="button" href={STORE}>{t.install}<ArrowUpRight size={18}/></a><p className="extension-note">{t.extensionNote}</p></div><div className="extension-preview"><img src={`${import.meta.env.BASE_URL}extension.png`} alt={lang==='en'?'Cost Per Use Chrome extension with an editable headphone example':'Düzenlenebilir kulaklık örneğiyle Cost Per Use Chrome eklentisi'} width="1280" height="800" loading="lazy"/><span><span className="status-dot"/>{t.free}</span></div></div></section>
